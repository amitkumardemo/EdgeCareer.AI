...
const SUB_STATUS = {
  ...
  NEEDS_REVISION: { label: "Revise", icon: AlertCircle, color: "text-orange-600 bg-orange-50" },
};
    ...
    <div className="max-w-6xl mx-auto px-4 py-8 space-y-10">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 text-lg mt-2 font-medium">Manage your internship deliverables and mentor submissions</p>
        </div>
        <div className="flex items-center gap-3 px-4 py-2 bg-indigo-50 border border-indigo-100 rounded-full">
           ...
           <span className="text-xs font-bold text-indigo-700 uppercase tracking-widest">Active Batch</span>
        </div>
      </div>
      ...
      {submitModal && (
        <div className="fixed inset-0 z-50 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white border border-slate-200 rounded-2xl p-8 w-full max-w-xl shadow-2xl">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-xl font-bold text-slate-900 flex items-center gap-2">
                 <Upload className="h-5 w-5 text-indigo-600" /> Submit Assignment
              </h2>
              <button onClick={() => setSubmitModal(null)} className="p-2 hover:bg-slate-100 rounded-full transition-colors">
            ...
            <div className="mb-6 p-4 bg-slate-50 border border-slate-100 rounded-xl">
               ...
               <p className="text-sm font-semibold text-slate-900">{submitModal.title}</p>
            </div>

            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="space-y-2">
                <Label className="text-sm font-bold text-slate-700">Deliverable URL</Label>
                <Input 
                  value={fileUrl} 
                  onChange={e => setFileUrl(e.target.value)} 
                  placeholder="https://github.com/your-repo or Google Drive link" 
                  required 
                  className="bg-white border-slate-200 text-slate-900 h-12 rounded-xl focus:ring-indigo-500/50" 
                />
              </div>
              <div className="space-y-2">
                ...
                <textarea 
                  ...
                  className="bg-white border border-slate-200 rounded-xl text-slate-900 text-sm p-4 w-full focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500 outline-none resize-none transition-all" 
                />
              </div>
              <Button type="submit" className="w-full h-12 text-base font-bold bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl shadow-md" disabled={submitting}>
      ...
      {loading ? (
        <div className="grid gap-6">{Array(3).fill(0).map((_, i) => <div key={i} className="animate-pulse bg-slate-100 border border-slate-200 rounded-2xl h-48" />)}</div>
      ) : apps.length === 0 ? (
        <div className="text-center py-24 bg-slate-50 border border-slate-200 rounded-3xl">
          <BookOpen className="h-12 w-12 mx-auto mb-4 text-slate-400" />
          <p className="text-xl font-bold text-slate-900 mb-2">No Verified Internships Found</p>
        ...
        <div className="grid gap-8">
          {tasks.map((task) => {
            ...
            return (
              <div key={task.id} className={`group bg-white border rounded-3xl p-6 md:p-8 transition-all duration-300 hover:shadow-lg ${overdue ? "border-red-200" : "border-slate-200 hover:border-indigo-300"}`}>
                <div className="flex flex-col lg:flex-row lg:items-start justify-between gap-8">
                  <div className="flex-1 space-y-4">
                    <div className="flex flex-wrap items-center gap-3">
                       <div className={`px-2.5 py-1 rounded-md text-[10px] font-black uppercase tracking-tighter ${overdue ? "bg-red-50 text-red-600" : "bg-indigo-50 text-indigo-600"}`}>
                    ...
                    <div className="relative">
                       <p className={`text-sm text-slate-600 leading-relaxed ${!isExpanded ? "line-clamp-3" : "whitespace-pre-wrap"}`}>
                         {renderDescriptionClean(task.description) || task.description}
                       </p>
                       {task.description?.length > 180 && (
                         <button 
                           onClick={() => toggleExpand(task.id)}
                           className="text-indigo-600 text-xs font-bold mt-2 hover:text-indigo-700 flex items-center gap-1"
                         >
                      ...
                      <div className="flex flex-wrap gap-2 pt-2">
                        {allLinks.map((link, idx) => (
                           <a 
                             ...
                             className="inline-flex items-center gap-2 px-4 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs font-bold text-slate-600 hover:bg-slate-100 hover:text-slate-900 transition-all"
                           >
                    ...
                    <div className="flex flex-wrap items-center gap-6 pt-4 border-t border-slate-100 mt-6">
                      <div className={`flex items-center gap-2 text-xs font-semibold ${overdue ? "text-red-500" : "text-slate-500"}`}>
                  ...
                  <div className="lg:w-64 flex flex-col gap-3">
                    {sub ? (
                      <div className="p-4 bg-slate-50 border border-slate-100 rounded-2xl space-y-3">
                        ...
                        {sub.fileUrl && (
                          <Button asChild variant="outline" className="w-full bg-white border-slate-200 text-slate-700 text-xs h-9 font-bold hover:bg-slate-50 hover:text-slate-900">
                      ...
                      <Button 
                        size="lg" 
                        className={`w-full font-bold rounded-2xl h-14 text-white ${overdue ? "bg-red-600 hover:bg-red-700 shadow-md shadow-red-600/10" : "bg-indigo-600 hover:bg-indigo-700 shadow-lg shadow-indigo-600/20"}`} 
                ...
                {sub?.feedback && (
                  <div className="mt-6 p-4 bg-indigo-50/50 border border-indigo-100 rounded-2xl relative overflow-hidden">
                    <div className="absolute top-0 right-0 p-2">
                       <MessageSquare className="h-12 w-12 text-indigo-500/10" />
                    </div>
                    <p className="text-sm font-bold text-indigo-700 mb-1 flex items-center gap-2 uppercase tracking-widest text-[10px]">
