...
export function CourseProgress({ selectedApp, progress }) {
  if (!selectedApp) {
    return (
      <motion.div 
        ...
        className="bg-white border border-slate-200 border-dashed rounded-3xl p-10 text-center mb-8 shadow-sm"
      >
        <div className="w-16 h-16 bg-slate-50 rounded-2xl flex items-center justify-center mx-auto mb-6">
          <BookOpen className="h-8 w-8 text-slate-400" />
        </div>
        <h2 className="text-xl font-bold text-slate-900 mb-2">No Active Internship</h2>
        ...
        <div className="flex flex-wrap justify-center gap-4">
          <Link href="/internship/student/apply">
            <Button className="bg-slate-900 border border-slate-800 hover:bg-slate-800 text-white rounded-xl h-12 px-8 font-bold">
  ...
  return (
    <motion.div 
      ...
      animate={{ opacity: 1, y: 0 }}
      transition={{ duration: 0.5, delay: 0.2 }}
      className="bg-white border border-slate-200 shadow-sm rounded-3xl p-8 mb-8 relative overflow-hidden group"
    >
      ...
      <div className="flex flex-col lg:flex-row justify-between gap-8 mb-8">
        <div className="flex-1">
          <div className="inline-flex items-center gap-2 px-3 py-1 bg-primary/10 text-primary border border-primary/20 rounded-full text-[10px] font-black uppercase tracking-widest mb-4">
            Active Internship
          </div>
          <h2 className="text-3xl font-black text-slate-900 leading-tight mb-2 group-hover:text-primary transition-colors">
      ...
      <div className="space-y-6">
        <div>
          <div className="flex justify-between text-xs font-bold text-slate-500 mb-2.5 uppercase tracking-wide">
            <span>Course Completion</span>
            <span className="text-slate-900">{progress?.progressPct}% Completed</span>
          </div>
        ...
        <div className="flex flex-wrap gap-3 mt-6">
          ...
          <Link href="/internship/student/attendance">
            <Button variant="outline" className="bg-slate-50 border-slate-200 hover:bg-slate-100 text-slate-700 rounded-xl px-6 h-10 text-xs font-bold shadow-sm">
