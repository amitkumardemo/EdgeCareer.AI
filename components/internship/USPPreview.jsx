        ...
        <div className="flex flex-col lg:flex-row items-center gap-16">
          ...
          <div className="flex-1 relative">
            ...
            <motion.div
              ...
              viewport={{ once: true }}
              className="relative w-full max-w-2xl bg-white rounded-24 md:rounded-32 p-4 md:p-6 shadow-2xl border border-slate-200 perspective-1000"
            >
              {/* Sidebar Mock */}
              <div className="flex gap-4 min-h-[400px]">
                <div className="w-12 md:w-16 h-full flex flex-col items-center gap-6 py-4 border-r border-slate-100">
                  <div className="btn-primary">T</div>
                  {[LayoutDashboard, ListTodo, BookOpen, Calendar, Award].map((Icon, i) => (
                    <Icon key={i} className={`w-5 h-5 ${i === 1 ? 'text-blue-500' : 'text-gray-400'}`} />
                  ))}
                </div>
                ...
                <div className="flex-1">
                  <div className="flex items-center justify-between mb-8 pb-4 border-b border-slate-100">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-xs text-gray-400 hidden md:inline">Quick search...</span>
                    </div>
                    <div className="flex items-center gap-4">
                      <Bell className="w-5 h-5 text-gray-400" />
                      <div className="flex items-center gap-2 px-3 py-1 rounded-full bg-blue-50 border border-blue-100">
                        <User className="w-4 h-4 text-blue-500" />
                        <span className="text-[10px] font-bold text-slate-800">Student ID: TECHIE-403</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4 mb-6">
                    <div className="p-4 rounded-2xl bg-slate-50 border border-slate-100">
                      ...
                      <p className="text-2xl font-bold text-slate-900 font-poppins">12/15</p>
                    </div>
                    <div className="p-4 rounded-2xl bg-gradient-to-br from-blue-50 to-purple-50 border border-slate-100">
                  ...
                  <div className="space-y-3">
                    <p className="text-xs font-bold text-gray-500 mb-2">Upcoming Tasks</p>
                    {[1, 2].map((i) => (
                      <div key={i} className="p-4 rounded-xl bg-white border border-slate-150 flex items-center justify-between group cursor-default">
                        <div className="flex items-center gap-3">
                          <div className={`w-2 h-2 rounded-full ${i === 1 ? 'bg-orange-500' : 'bg-blue-500'}`} />
                          <p className="text-xs text-slate-800 font-medium">Task {i === 1 ? '7: API Integration' : '8: Deploy to Production'}</p>
                        </div>
                        <span className="text-[10px] text-slate-500 px-2 py-1 rounded bg-slate-50">0d left</span>
                      </div>
                    ))}
                  </div>
                </div>
