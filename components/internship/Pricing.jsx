...
const Pricing = () => {
  return (
    <section id="pricing" className="py-24 bg-slate-50 px-6 relative overflow-hidden">
          ...
          {plans.map((plan, i) => (
            <motion.div
              ...
              className={`relative flex flex-col p-8 rounded-[2.5rem] border h-full transition-all duration-500 overflow-hidden ${
                plan.popular 
                ? 'bg-white border-orange-500 z-10 shadow-xl' 
            ...
            >
              ...
              <div className="mb-8">
                <p className="text-gray-500 font-bold mb-2 uppercase tracking-widest text-xs">{plan.name}</p>
                <div className="flex items-baseline gap-1">
                  <span className="text-gray-900 text-4xl font-black">₹{plan.price}</span>
              ...
              <div className="flex-1 space-y-4 mb-10">
                {plan.features.map((feature, j) => (
                  <div key={j} className="flex items-start gap-4">
                    <div className={`mt-1 p-1 rounded-full ${plan.popular ? 'bg-orange-500' : 'bg-slate-100'}`}>
                      <Check className={`w-3 h-3 ${plan.popular ? 'text-white' : 'text-slate-600'}`} />
                    </div>
                    <span className={`text-base font-medium leading-tight ${plan.popular ? 'text-slate-800 font-semibold' : 'text-slate-600'}`}>{feature}</span>
                  </div>
                ))}
              </div>
